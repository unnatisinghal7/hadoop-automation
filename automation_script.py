...
while True:
	...
	if int(i)==1:
		...
		file.write("""export JAVA_HOME=/usr/java/jdk1.7.0_51/
...
<value>{1}:9002</value>
				...
				l=raw_input("Enter your Choice: ")
				if int(l)==1:
					os.system("sshpass -p {0} scp /root/Desktop/Project/hive.tar.gz {1}:/".format(password,Ip))
					os.system("sshpass -p {0} ssh {1} tar -xvf /hive.tar.gz".format(password,Ip))
					os.system("sshpass -p {0} ssh {1} mv /root/apache-hive-1.2.1-bin/ /hive/".format(password,Ip))
